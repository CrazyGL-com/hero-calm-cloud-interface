...
function buildCloudCanvas(
	...
	function hash2(ix: number, iy: number, s: number) {
	...
	function fbm4(x: number, y: number, s: number) {
	...
	function fbm3(x: number, y: number, s: number) {
		let amp = 0.5, freq = 1.0, sum = 0, norm = 0;
		for (let o = 0; o < 3; o++) {
...
function smoothstep01(edge0: number, edge1: number, v: number) {
	const t = Math.max(0, Math.min(1, (v - edge0) / (edge1 - edge0)));
	return t * t * (3 - 2 * t);
}

function CalmCloudHero(props: HeroComponentProps) {
	const {
		size,
		input,
		reducedMotion,
	...
	const threeModuleRef = React.useRef<Promise<any> | null>(null);
	...
	const stateRef = React.useRef<any>({
		...
		// Clouds.
		cloudGroup: null,
		clouds: [] as Array<any>,        // { mesh, baseX, baseY, z, speedFactor, parallaxFactor, halfW, halfH, texture, material }
		cloudColorKey: '',
		lastCloudCount: -1,
	...
	React.useEffect(() => {
		...
		(async () => {
			const THREE: any = await threeModuleRef.current;
			...
			function rebuildClouds() {
				...
				for (const c of state.clouds) {
					try { cloudGroup.remove(c.mesh); } catch {}
					try { c.mesh.geometry?.dispose?.(); } catch {}
					try { c.material?.dispose?.(); } catch {}
					try { c.texture?.dispose?.(); } catch {}
				}
				state.clouds = [];
			...
			function applyScreenLayout() {
				if (!state.screenMesh || !state.screenGroup) return;
		...
		return () => {
			...
			state.clouds = [];
			try { state.skyMesh?.geometry?.dispose?.(); } catch {}
			try { state.skyMat?.dispose?.(); } catch {}
			try { state.screenMesh?.geometry?.dispose?.(); } catch {}
			try { state.screenMat?.dispose?.(); } catch {}
			try { state.screenshotTex?.dispose?.(); } catch {}
	...
	React.useEffect(() => {
		...
		try { state.hemi.color.set(skyTop); } catch {}
		try { state.hemi.groundColor.set(skyBottom); } catch {}
	...
	React.useEffect(() => {
		if (!screenshot || typeof Image === 'undefined') return;
		const img = new Image();
	...
	React.useEffect(() => {
		...
		if (state.screenshotKey === screenshot) return;
		...
		loader.load(
			...
			(tex: any) => {
				...
				if (state.screenshotTex) state.screenshotTex.dispose?.();
	...
	useHeroAnimationFrame(props.rootRef, ({ delta }) => {
		const state = stateRef.current;
		if (!state.ready || !state.renderer || !state.scene || !state.camera) return;
	...
	return (
		<>
			<crazygl-stage
				style={{ position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden' } as React.CSSProperties}
			>
				<canvas ref={canvasRef} className="crazygl-cloud-canvas" aria-hidden="true" />
			</crazygl-stage>
			<crazygl-content
				style={
					{
			...
			>
				<div className="crazygl-cloud-content">{content.node}</div>
			</crazygl-content>
		</>
	);
}

export default function CalmCloudInterface(props: any) {
	return <CrazyGLWrapper hero={CalmCloudHero} metadata={metadata as any} {...props} />;
}
